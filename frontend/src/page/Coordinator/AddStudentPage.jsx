...
import { useBatches, useAddStudentToBatch } from '@/hooks/useBatch.js';
import { flexRender, getCoreRowModel, useReactTable } from '@tanstack/react-table';
import { useGetStudentsWithoutBatch } from '@/hooks/useAuth.js';
import toast from 'react-hot-toast';
import { UserPlus, ArrowLeft, School, Search, Info } from "lucide-react";
import { Link } from 'react-router-dom';

const studentColumns = (handleAddStudent, selectedBatchId) => [
    {
        accessorKey: 'name',
        header: 'Name',
    },
...
const DataTable = ({ columns, data, filterValue, setFilterValue }) => {
    ...
    const filteredData = filterValue 
        ? data.filter(student => 
            ...
            student.rollNo?.toLowerCase().includes(filterValue.toLowerCase()) ||
            student.section?.toLowerCase().includes(filterValue.toLowerCase())
          )
        : data;
        ...
        <div className="space-y-4">
            <div className="relative">
                <input
                    ...
                    className="w-full py-2 pl-10 pr-4 bg-base-100 rounded-lg border border-gray-300 focus:border-purple-500 focus:ring-2 focus:ring-purple-200"
                />
                                ...
                                <tr>
                                    <td colSpan={columns.length} className="px-6 py-8 text-center text-sm text-base-content/70">
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <p>No students without a batch found</p>
                                            <p className="text-xs text-base-content/50">All students are already assigned to batches</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
...
const AddStudentPage = () => {
    ...
    const { data: students, isLoading: isLoadingStudents, error: errorStudents } = useGetStudentsWithoutBatch();
    const addStudentToBatch = useAddStudentToBatch();
    const [selectedBatchId, setSelectedBatchId] = useState('');
    const [filterValue, setFilterValue] = useState('');

    const handleAddStudent = (studentId) => {
        if (selectedBatchId) {
            addStudentToBatch.mutate(
                { studentId, batchId: selectedBatchId },
                {
                    onError: (error) => {
    ...
    if (isLoadingBatches || isLoadingStudents) return (
        <div className="min-h-[60vh] flex items-center justify-center">
            <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-purple-500"></div>
        </div>
    );
    
    if (errorBatches || errorStudents) return (
        <div className="min-h-[60vh] flex items-center justify-center">
            ...
            <div className="bg-base-100 rounded-2xl shadow-xl p-4 sm:p-6 mb-6 sm:mb-8">
                ...
                {!selectedBatchId && (
                    <p className="text-sm text-amber-600 mt-2 flex items-center gap-1">
                        ...
                        Please select a batch to add students
                    </p>
                )}
            </div>
